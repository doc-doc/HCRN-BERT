...
def validate(cfg, model, data, device, write_preds=False):
    ...
    with torch.no_grad():
        for batch in tqdm(data, total=len(data)):
            video_ids, question_ids = batch[0], batch[1]
            answers, *batch_input = [todevice(x, device) for x in batch[2:]]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', dest='cfg_file', help='optional config file', default='configs/nextqa.yml', type=str)
    args = parser.parse_args()
    if args.cfg_file is not None:
        cfg_from_file(args.cfg_file)

    assert cfg.dataset.name in ['tgif-qa', 'msrvtt-qa', 'msvd-qa', 'nextqa']
    ...
    elif cfg.dataset.name == 'nextqa':
        cfg.dataset.test_question_pt = os.path.join(cfg.dataset.data_dir, "val.pt")
        cfg.dataset.vocab_json = os.path.join(cfg.dataset.data_dir, "train_vocab.json")
        cfg.dataset.app_feat_dir = os.path.join(cfg.dataset.feat_dir, "HCRN/appearance")
        cfg.dataset.mot_feat_dir = os.path.join(cfg.dataset.feat_dir, "HCRN/motion")
    else:
    ...
    test_loader_kwargs = {
        ...
        'app_feat_dir': cfg.dataset.app_feat_dir,
        'mot_feat_dir': cfg.dataset.mot_feat_dir,
    ...
    if cfg.test.write_preds:
        ...
        preds_file = os.path.join(output_dir, "val_preds.json")
        print('save to {}'.format(preds_file))
        if cfg.dataset.question_type in ['action', 'transition']: \
            ...
            instances = {q_id: {'question': ' '.join([vocab[word.item()] for word in dict[str(q_id)][1] if word != 0]),
                 'answer': answer,
                 'prediction': pred} for q_id, answer, pred in
                zip(np.hstack(q_ids).tolist(), gts, preds)}
            print(len(instances))
            ...
            # Display 10 samples
            for idx in range(10):
                print('Video name: {}'.format(dict[str(q_ids[idx])][0]))
                cur_question = [vocab[word.item()] for word in dict[str(q_ids[idx])][1] if word != 0]
                print('Question: ' + ' '.join(cur_question) + '?')
                all_answer_cands = dict[str(q_ids[idx])][2]
                for cand_id in range(len(all_answer_cands)):
                    cur_answer_cands = [vocab[word.item()] for word in all_answer_cands[cand_id] if word != 0]
