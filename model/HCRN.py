...
class InputUnitLinguistic(nn.Module):
    def __init__(self, vocab_size, wordvec_dim=300, rnn_dim=512, module_dim=512, bidirectional=True):
        ...
        self.embedding = nn.Linear(768, wordvec_dim)
        self.tanh = nn.Tanh()
    ...
    def forward(self, questions, question_len):
        ...
        questions_embedding = self.embedding(questions)  # (batch_size, seq_len, dim_word)
...
class OutputUnitMultiChoices(nn.Module):
    ...
    def forward(self, question_embedding, q_visual_embedding):
        ...
        out = torch.cat([q_visual_embedding, question_embedding], 1)
        out = self.classifier(out)
...
class HCRNNetwork(nn.Module):
    def __init__(self, vision_dim, module_dim, word_dim, k_max_frame_level, k_max_clip_level, spl_resolution, vocab, question_type):
        ...
        if self.question_type in ['action', 'transition']:
            encoder_vocab_size = 0 #for BERT #len(vocab['question_answer_token_to_idx'])
    ...
    #             question_len):
    def forward(self, video_appearance_feat, video_motion_feat, question, question_len):
        """
        Args:
        ...
        if self.question_type in ['frameqa', 'count', 'none']:
            ...
            # question = question.permute(1, 0, 2, 3)
            # question_len = question_len.permute(1, 0)
            # outs = []
            # for i in range(question.shape[0]):
            #     question_embedding = self.linguistic_input_unit(question[i], question_len[i])
            #     visual_embedding = self.visual_input_unit(video_appearance_feat, video_motion_feat, question_embedding)
            #
            #     q_visual_embedding = self.feature_aggregation(question_embedding, visual_embedding)
            #
            #     out = self.output_unit(question_embedding, q_visual_embedding)
            #     outs.append(out)
            #
            # out = torch.stack(outs, 0).transpose(1, 0).reshape(-1, 1)


            # ans_candidates: (batch_size, num_choices, max_len)
            question_agg = question.view(-1, question.size(2), question.size(3))
            question_len_agg = question_len.view(-1)

            batch_agg = np.reshape(
                np.tile(np.expand_dims(np.arange(batch_size), axis=1), [1, 5]), [-1])

            question_embedding = self.linguistic_input_unit(question_agg, question_len_agg)

            visual_embedding = self.visual_input_unit(video_appearance_feat[batch_agg], video_motion_feat[batch_agg], question_embedding)
